...
par(mfrow=c(1,3))
for(i in 1:ncols){
  Acf(ts_energy_data2[,i],lag.max=40,main=datanames[i],cex.main=0.5,ylim=c(-1,1),ylab='Acf')
}
for(i in 1:ncols){
  Pacf(ts_energy_data2[,i],lag.max=40,main=datanames[i],cex.main=0.5,ylim=c(-1,1),ylab='Pacf')
}
```
...
Answer:
Yes, all of them have a trend. Both the total biomass energy production and the total renewable energy production have a upward trend, and the hydroelectric power consumption has a slight downward trend.
