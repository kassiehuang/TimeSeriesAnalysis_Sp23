...
# KH: with respect to the working directory <referring to the knit directory> 
...
#Checking data
head(raw_inflow_data) #first six rows of the data
str(raw_inflow_data) 
```
## Date or time period
...
#m-for month, y-for year, d-for days
head(my_date)
#add that to inflow_data 
inflow_data <- cbind(my_date,raw_inflow_data[,3:5]) #column binding -> output: data.frame
head(inflow_data)
...
#KH: the pattern: log-normal distripution
...
#KH: color=reservoir <-  to substitue the default name "series"
ggplot(inflow_data, aes(x=my_date, y=Inflow)) +
            geom_line(aes(y=HP1,col="HP1")) +
            geom_line(aes(y=HP2,col="HP2")) +
...
# [,x] taking all the rows
# [,2:(2+nhydro-1)] from column 2 to column (2+nhydro-1)
# can also use: 
#ts(inflow_data[,"HP2"])
#ts(inflow_data$HP2)
ts_inflow_data
...
pacf(ts_inflow_data[,1])
#x-axies:time series
#blue dotted lines: significant levels
#acf will only run for time series objects
