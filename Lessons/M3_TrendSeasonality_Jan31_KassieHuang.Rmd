...
#Kassie: （2+nhydro-1）意味着，从2开始，往后数nhydro列，“-1”用于调整计数（2+15=17，实际上一共只有16列）
#start=c(1931,1,1): 第一个数据从1931.1.1开始；c(1931,3):第一个数据从1931.3开始
head(ts_inflow_data)
```
...
for(i in 1:nhydro){
  par(mfrow=c(1,2))
  Acf(ts_inflow_data[,i],lag.max=40,main=paste("HP",i))
  Pacf(ts_inflow_data[,i],lag.max=40,main=paste("HP",i))
}

#Alternative: directly extracting the column names to be the title of the plot
name <- colnames(ts_inflow_data)
for(i in 1:nhydro){
  par(mfrow=c(1,2))
  Acf(ts_inflow_data[,i],lag.max=40,main=name[i])
  Pacf(ts_inflow_data[,i],lag.max=40,main=paste("HP",i))
}

#Alternative 2: set a fixed y-axis range
name <- colnames(ts_inflow_data)
for(i in 1:nhydro){
  par(mfrow=c(1,2))
  Acf(ts_inflow_data[,i],lag.max=40,main=name[i],ylim=c(-1,1))
  Pacf(ts_inflow_data[,i],lag.max=40,main=paste("HP",i))
}

#comment: the choice of lag.max=40 is for demonstration
...
linear_trend <- lm(ts_inflow_data[,iHP] ~ t)
summary(linear_trend)

ggplot(inflow_data,aes(x=my_date,y=ts_inflow_data[,iHP]))+
  geom_line(color='blue')+
